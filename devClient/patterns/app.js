...
var styleFile = function (fileObj) {
    ...
    return new Promise(function (resolve, reject) {
        ...
        var styleFileDownload = function () {
            ...
            fetchish(fileUrl).then(function (response) {
                guideLog(fileType + ' file ' + fileName + ' - downloaded');
                styleFilePreProcess(response);
            }, function (error) {
                guideLog(fileType + ' file ' + fileName + ' - could not be downloaded. path: ' + fileUrl, 'error');
                return;
            });
        }
        ...
        var styleFilePreProcess = function (cssFile) {
            ...
            guideLog(fileType + ' file ' + fileName + ' - ' + lineCount + ' lines');
        ...
        var styleFileProcess = function (cssArray) {
            ...
            function styleBlockLineProcess(lineString) {
                ...
                if (codeState === true && lineString.indexOf('        ') === -1) {
                    ...
                    blockLineArray.push('<button class="style-toggle js-style-toggle"><span class="style-toggle__closed-text">Show code</span><span class="style-toggle__open-text">Hide code</span></button>');
                    blockLineArray.push('    ');
            ...
            if (blocksCounter) {
                guideLog(fileType + ' file ' + fileName + ' - ' + ' CONTENT FOUND (' + blocksCounter + ' block' + (blocksCounter !== 1 ? 's' : '') + ')', 'yes');
                return;
            } else {
                guideLog(fileType + ' file ' + fileName + ' - ' + ' NO CONTENT', 'no');
                return;
            }
        }
...
var styleJsonStore = function (blockJson) {
    ...
    guideLog(blockJson.fileType + ' file ' + blockJson.fileName + '#' + blockJson.section + ' - ' + ' stored');
}
